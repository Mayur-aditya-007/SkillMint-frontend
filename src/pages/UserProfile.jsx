...
export default function UserProfile() {
  ...
  // shape data to match MyProfile UI, prefer user's data with safe fallbacks
  const profile = useMemo(() => {
    ...
    return {
      ...
      name,
      handle: user?.handle || `@${(user?.email || "user").split("@")[0]}`,
      ...
      skills: Array.isArray(user?.skills) && user.skills.length ? user.skills : ["React", "Node.js", "Tailwind CSS"],
      goals: Array.isArray(user?.goals) && user.goals.length
        ? user.goals
        : ["Learn something new every day", "Build a portfolio project"],
      socials: Array.isArray(user?.socials) && user.socials.length
        ? user.socials
        : [
      ...
      stats: user?.stats || {
        ...
        communityPoints: user?.stats?.communityPoints ?? 0,
      },
      enrolled: Array.isArray(user?.enrolled) ? user.enrolled : [],
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="mt-16" />
      {loading ? (
        <div className="max-w-7xl mx-auto px-6 py-16 text-white/70">Loading…</div>
      ) : !user ? (
        <div className="max-w-7xl mx-auto px-6 py-16 text-white/70">User not found</div>
      ) : (
        <>
          {/* cover + avatar header (same vibe as MyProfile) */}
            ...
            <div className="max-w-7xl mx-auto px-6 -mt-16 md:-mt-20 relative z-10">
              <div className="bg-white/10 backdrop-blur border border-white/10 rounded-2xl p-4 md:p-6 flex flex-col md:flex-row items-start md:items-end gap-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm">{profile.handle}</p>
                  <div className="flex flex-wrap gap-2 mt-3">
                    <Badge>{profile.role}</Badge>
                    {profile.location !== "—" && <Badge>{profile.location}</Badge>}
                    <Badge subtle>{profile.email}</Badge>
                  </div>
                </div>

                {/* actions */}
                <div className="flex items-center gap-2 md:gap-3">
                  <GhostButton onClick={() => navigate(`/messages?peerId=${id}`)}>Message</GhostButton>
          ...
          <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              ...
              <Card title="Enrolled Courses">
                {profile.enrolled.length === 0 ? (
                  <p className="text-gray-300 text-sm">No courses yet.</p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {profile.enrolled.map((c) => (
                      <CourseMini key={c.id || c._id} course={c} />
                    ))}
                  </div>
                )}
              </Card>
...
/* ---------- small building blocks (same style as MyProfile) ---------- */
...
function CourseMini({ course }) {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-4">
      ...
      <button className="mt-3 w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm">
        Continue
      </button>
    </div>
  );
